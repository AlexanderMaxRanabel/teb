...
if not os.path.exists(H5_PATH) or os.path.getsize(H5_PATH) < 10_000_000:
    print("Downloading ModelNet10 (72.5 MB) via wget...")
    subprocess.run(["wget", "-q", "--show-progress", MODELNET10_URL, "-O", H5_PATH], check=True)

# Verify it's actually HDF5 (magic bytes: \x89HDF)
with open(H5_PATH, 'rb') as f:
    magic = f.read(8)
if not magic.startswith(b'\x89HDF'):
    raise RuntimeError(f"Download failed — got {magic[:20]!r}, not HDF5. Delete {H5_PATH} and retry.")

print(f"Dataset OK ({os.path.getsize(H5_PATH)/1e6:.1f} MB)")
