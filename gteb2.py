...
if LITE_MODE:
    ...
    LR = 1e-3
    NUM_GLOBAL = 16
    print(f"Running in FULL MODE (GPU optimized) - GTEB v2, NUM_GLOBAL={NUM_GLOBAL}")
...
# ==========================================
# 3. GTEB v2: Globulus TEB Layer (Fixed)
# ==========================================
class GlobulusTEBLayer(nn.Module):
    def __init__(self, dim, num_global=16, steps=3, k=16, sigma=24.0, radius=0.0):
        ...
        # Projections for the new context mechanism
        self.global_to_local_hint = nn.Linear(dim, dim)
        self.local_to_global_update = nn.Linear(dim, dim)
    ...
    def forward(self, x):
        ...
        for step in range(self.steps):
            # 1. Inject global context into local tokens (Token-Specific via kNN)
            # Compute distance between local tokens and globulus tokens
            dist_sq_hint = torch.cdist(x, globulus) ** 2 # [B, N, num_global]
            
            # Gaussian weights for attention over globulus tokens
            W_hint = torch.exp(-dist_sq_hint / (2 * self.sigma**2)) # [B, N, num_global]
            W_hint = W_hint / (W_hint.sum(dim=-1, keepdim=True) + 1e-8) # Normalize
            
            # Weighted sum of globulus tokens for each local token
            context = torch.einsum('bnk,bkd->bnd', W_hint, globulus) # [B, N, D]
            global_hint = self.global_to_local_hint(context) # Project the context
            
            x_with_context = x + global_hint # Token-specific global context
            
            # 2. Run the precise local TEB process
            x_updated = self.local_teb(x_with_context) # [B, N, D]
            
            # 3. Update the Globulus Map based on new local state (Structure-Preserving)
            # Globulus tokens attend to local tokens
            dist_sq_global = torch.cdist(globulus, x_updated) ** 2 # [B, num_global, N]
            k_global = min(self.k, N)
            
            # Find kNN of local tokens for each globulus token
            knn_dist_g, knn_idx_g = torch.topk(dist_sq_global, k_global, largest=False, dim=-1) # [B, num_global, k_global]
            
            # Gaussian weights
            W_g = torch.exp(-knn_dist_g / (2 * self.sigma**2)) # [B, num_global, k_global]
            W_g = W_g / (W_g.sum(dim=-1, keepdim=True) + 1e-8) # Normalize
            
            batch_idx = torch.arange(B, device=x.device).view(B, 1, 1).expand(B, self.num_global, k_global)
            # Gather local tokens based on globulus kNN indices
            x_neighbors_global = x_updated[batch_idx, knn_idx_g] # [B, num_global, k_global, D]
            
            # Weighted sum to form update
            globulus_update = torch.einsum('bgk,bgkd->bgd', W_g, x_neighbors_global) # [B, num_global, D]
            
            # Update the Globulus Map
            globulus = globulus + self.local_to_global_update(globulus_update)
...
# ==========================================
print(f"\nTraining GTEB v2 (Fixed Globulus TEB) - {NUM_GLOBAL} global tokens...")
