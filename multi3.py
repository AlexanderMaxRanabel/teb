...
# ==========================================
# 🚀 TEB: MULTI3 (Three Stacked Layers)
# ==========================================
LITE_MODE = True 
if LITE_MODE:
...
# Hyperparameters (Balanced for 3 Layers)
if LITE_MODE:
    DATA_SIZE = 180000
    SEQ_LEN = 32
    BATCH_SIZE = 8
    EPOCHS = 1500
    DIM = 40
    STEPS = 2         # 2 steps per layer = 6 total steps
    K_NEIGHBORS = 4   # Keeping your best setting
    SIGMA = 1.0       
    RADIUS = 0.1      
    LR = 5e-4         
    print("Running in LITE MODE (Termux optimized) - 3 Stacked Layers")
else:
    DATA_SIZE = len(text) 
    SEQ_LEN = 128
    BATCH_SIZE = 64
    ...
    RADIUS = 0.3
    LR = 5e-4
    print("Running in FULL MODE (GPU optimized) - 3 Stacked Layers")
...
class SparseIterativeEmergenceLayer(nn.Module):
    def __init__(self, dim, steps=2, k=4, sigma=1.0, radius=0.1):
        super().__init__()
        self.dim = dim
        self.steps = steps
        self.k = k
...
class SparseEmergenceLM(nn.Module):
    def __init__(self, vocab_size, dim=40, max_len=32, steps=2, k=4, sigma=1.0, radius=0.1):
        super().__init__()
...
class TransformerLM(nn.Module):
    def __init__(self, vocab_size, dim=40, nhead=4, max_len=32):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, dim)
        self.pos_embed = nn.Embedding(max_len, dim)
...
def train_model(model, data, epochs, batch_size, seq_len, device, lr=5e-4):
...
# 7. INT8 Quantization
# ==========================================
print("\nApplying INT8 Quantization to TEB (qnnpack)...")
torch.backends.quantized.engine = 'qnnpack'
emergence_model_quantized = torch.quantization.quantize_dynamic(
    emergence_model, {nn.Linear}, dtype=torch.qint8
)

print("\n--- GENERATED TEXT (TEB - 3 Layers) ---")
print(generate(emergence_model_quantized, device=device))
